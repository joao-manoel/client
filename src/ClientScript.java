...
public class ClientScript extends Class298_Sub37 {
	...
	try {
		...
		if (string.equalsIgnoreCase("runescape") || string.contains("runescape")) {
		    string = string.replace("runescape", "Ressurge");
		}
		if (string.contains("\r\n" + "Sua conta foi bloqueada")) {
		    string = string.replace("\r\n" + "Sua conta foi bloqueada. Se você não recebeu um e-mail de recuperação da conta, selecione 'Recover Account'.", "Sua conta foi bloqueada devido a suspeita. Você pode recuperá-lo entrando em contato com algum <img=1> Administrador.");
		}
		if (string.equalsIgnoreCase("\r\n" + "Para contas criadas após 24 de novembro de 2010, use seu endereço de e-mail para fazer login. Caso contrário, faça o login com seu nome de usuário.")) {
		    string = string.replace("For accounts created after the 24th of November 2010, please use your email address to login. Otherwise please login with your username.", "Para contas criadas após 24 de novembro de 2010, use seu endereço de e-mail para fazer login. Caso contrário, faça o login com seu nome de usuário. \",\" Seu nome de usuário ou senha estavam incorretos. Verifique os erros de digitação em qualquer um deles. Se você acredita que foi invadido, entre em contato com um administrador <img = 1> para obter mais assistência.");
		}
		if (string.equalsIgnoreCase("Recuperar a conta")) {
		    string = string.replace("Recover Account", "Contact <img=1>Admin");
		}
